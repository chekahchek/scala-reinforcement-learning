...
trait Exploration[E <: Env[F], F[_]] {
  def getAction(actionSpace: List[E#Action],
                state: E#State,
                qValues: Map[(E#State, E#Action), Double]): F[E#Action]
}

  case class EpsilonGreedy[E <: Env[IO]](explorationRate: Double) extends Exploration[E, IO] {
    override def getAction(actionSpace: List[E#Action],
                           state: E#State,
                           qValues: Map[(E#State, E#Action), Double]): IO[E#Action] = for {
  ...
  case class UCB[E <: Env[IO]](constant: Int,
                               ucbRef: Ref[IO, Map[(E#State, E#Action), Int]]) extends Exploration[E, IO] {

    override def getAction(actionSpace: List[E#Action],
                           state: E#State,
                           qValues: Map[(E#State, E#Action), Double]): IO[E#Action] = for {
      stateActionCount <- ucbRef.get
      totalCounts = actionSpace.map(a => stateActionCount.getOrElse((state, a), 0)).sum
      action <- IO.pure {
        actionSpace.maxBy { a =>
