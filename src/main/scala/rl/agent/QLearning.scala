...
class QLearning[E <: Env[IO]](val env: E,
  ...
  def act(state: env.State): IO[env.Action] = for {
    ...
    qValues <- qTable.get
    action <- if (scala.util.Random.nextDouble() < explorationRate) {
      // Explore: choose a random action
      IO.pure(actionSpace(scala.util.Random.nextInt(actionSpace.length)))
    } else {
  ...
  def runStep(state: env.State): IO[Boolean] = for {
    ...
    maxNextQ = if (done) 0.0 else actionSpace.map(a => qValues.getOrElse((nextState, a), 0.0)).max // To check
